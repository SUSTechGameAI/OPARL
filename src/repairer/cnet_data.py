...
def generate_cnet_data():
    lvl_files = glob.glob(get_path('levels/train/*.txt'))
    levels = [MarioLevel.from_txt(lvl_file) for lvl_file in lvl_files]
    centers, surroundings = [], []
    for lvl in levels:
    ...
    with open(get_path('levels/train/cnet_data.json'), 'w') as f:
        json.dump(data, f)
    pass
