...
from src.utils import batched_iter, get_path


def get_gan_train_data():
    H, W = MarioLevel.height, MarioLevel.default_seg_width
    path = get_path('levels/train')
    data = []
    for fpath in glob.glob(f'{path}/*.txt'):
        num_lvl = MarioLevel.from_txt(fpath).to_num_arr()
...
def set_parser(parser):
    ...
    parser.add_argument('--niter', type=int, default=2000, help='Number of epochs to train GAN')
...
def train_gan(cfgs):
    ...
    os.makedirs(get_path(res_path), exist_ok=True)
    ...
    for t in range(cfgs.niter):
        ...
        if t % 50 == 49:
            ...
            with torch.no_grad():
                z = torch.randn(100, nz, 1, 1, device=device)
                fake = netG(z)
            levels = process_levels(fake[:20], True)
            iteration_path = res_path + f'/iteration{t+1}'
            os.makedirs(get_path(iteration_path), exist_ok=True)
            for i, lvl in enumerate(levels):
                lvl.save(iteration_path + f'/sample{i}.txt')
                lvl.to_img(iteration_path + f'/sample{i}.png')
            torch.save(netG, get_path(f'{iteration_path}/generator.pth'))
            torch.save(netG.state_dict(), get_path(f'{iteration_path}/generator.pth'))
            netD.train()
            netG.train()
            pass
